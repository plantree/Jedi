...
var block = {
  ...
  text: /^[^\n]+/,
  toc: /^\[TOC\]\n/,       //added by cj -- for [TOC]
  meta:/^-{3,}\s*?\n([\s\S]*?)\n-{3,}/,     //added by cj -- for [meta header]
  metaItem:/(\S+)\s*?:\s*([\s\S]*?)(?=$|\n)/g,  //added by cj -- for [meta header item]
...
//added by cj -- for [TOC,meta header]
...
Lexer.prototype.token = function(src, top, bq) {
  ...
  while (src) {
    ...
    //meta -- added by cj -- for [meta header]
  ...
  if(tocIndex>=0 || this.options.reHeader){
    ...
    for(var i=0;i<this.tokens.length;i++){
      if(this.tokens[i].type==='heading'){
        ...
        this.tokens[i].index=toc.length;   // added by cj -- for [heading id]
      }
    }
    this.tokens[tocIndex]={type:"toc",datas:toc};
    //console.log(this.tokens[tocIndex]);
  }
  return this.tokens;
};
  ...
  switch (this.token.type) {
    ...
    case 'heading': {
      return this.renderer.heading(
        this.inline.output(this.token.text),
        this.token.depth,
        this.token.text,this.token.index);  //added by cj -- for [heading id]
    }
    //added by cj -- for [TOC]
    case 'toc':{
      ...
      //console.log(datas);
      var tocStr= this.renderer.toc(datas);
      //console.log(tocStr);
      return tocStr;
    }
    //added by cj -- for [meta header]
    case 'meta':{
      return "";
    }
